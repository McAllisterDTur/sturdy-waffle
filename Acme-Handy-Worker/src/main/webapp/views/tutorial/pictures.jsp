...
<jstl:if test="${tutorial.worker.account.username == principal}">
	<script type="text/javascript">
		function add(id){
			...
			window.location.href='tutorial/pictures/handyworker/add.do?id=' + id + '&picture=' + picture;
		}
	</script>
...
<display:table pagesize="5" name="tutorial.photoURL" id="row" requestURI="${requestURI}">
	...
	<jstl:if test="${tutorial.worker.account.username == principal}">
		<display:column>
			<button onClick="window.location.href='tutorial/pictures/handyworker/delete.do?id=${tutorial.id }&picture=${row}'"><spring:message code="tutorial.pictures.delete"/></button>
		</display:column>
	</jstl:if>
	</security:authorize>
</display:table>
