...
public interface CustomerRepository extends JpaRepository<Customer, Integer> {
	...
	@Query("select distinct c from Customer c join c.fixUpTasks p join p.applications a where ((select count(f) from FixUpTask f join f.customer q where q.id = c.id) >= 1.1*((select count(task) from FixUpTask task)*1.0/(select count(y) from Customer y)*1.0)) order by (a.size) desc")
